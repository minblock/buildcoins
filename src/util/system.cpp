...
const char * const BITCOIN_CONF_FILENAME = "buildcoins.conf";
...
#else
    const char* pszModule = "buildcoins";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Buildcoins";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Buildcoins";
#else
    // Unix
    return pathRet / ".buildcoins";
#endif
#endif
}
