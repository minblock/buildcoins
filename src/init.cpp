 ...
 */
static const char* BITCOIN_PID_FILENAME = "buildcoinsd.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("buildcoins-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/buildcoins-project/buildcoins>";
    const std::string URL_WEBSITE = "<https://buildcoins.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("buildcoins-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if buildcoins is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if buildcoins is started while in a temporary directory.\n",
